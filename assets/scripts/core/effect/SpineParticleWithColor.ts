...
class Particle{
    ...
    update(){
        ...
        this.node.scale = 2;//0.5 * this.size / this._node.width;
...
@ccclass
export class SpineParticleWithColor extends cc.Component{
    ...
    createParticle(){
        ...
        let colors = [cc.color(252, 255, 35), cc.color(35, 255 , 26), cc.color(255, 74, 74)];
        let index = Math.floor(Math.random() * colors.length);
        let pNode = new cc.Node();
        pNode.color = colors[index];
