...
export class EngineTime extends EventDispatcher {
    ...
    public schedule(callback: Function, interval: number):string{
        let UUID = `schedule+${this._scheduleCount++}`
    ...
    public scheduleOnce(callback: Function, delay: number = 0):string {
        let UUID = `scheduleOnce+${this._scheduleCount++}`;
        this.schedules[UUID] = callback;
    ...
    onUpdate(dt: number) {
        // 后台管理倒计时完成事件
        for (let key in EngineTime.times) {
            let data = EngineTime.times[key];
    ...
    public save() {
        for (let key in EngineTime.times) {
            EngineTime.times[key].startTime = this.getTime();
        }
    }
    ...
    public load() {
        for (let key in EngineTime.times) {
            let interval = Math.floor((this.getTime() - (EngineTime.times[key].startTime || this.getTime())) / 1000);
            let data = EngineTime.times[key];
            data.object[data.field] = data.object[data.field] - interval;
            if (data.object[data.field] < 0) {
                data.object[data.field] = 0;
            }
            EngineTime.times[key].startTime = null;
        }
    }
    ...
    public registerObject(object: any, field: string, onSecond: Function, onComplete: Function) {
        ...
        EngineTime.times[data.id] = data;
        return data.id;
    }

    /** 注消指定对象的倒计时属性更新 */
    public unRegisterObject(id) {
        if (EngineTime.times[id])
            delete EngineTime.times[id];
    }
    ...
    public unRegister(event: string) {
        if (EngineTime.times[event])
            delete EngineTime.times[event];
    }
}
...
export class Timer {
    ...
    constructor(step: number = 1) {
        this.step = step;                     // 每次修改时间
        this.elapsedTime = 0;                 // 逝去时间
    }
